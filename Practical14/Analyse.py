...
def parse_xml_dom(file):  # Parse the XML file using DOM parser and print the number of terms
    ...
    id_map = {}  # Map term names to IDs for output
    for node in terms:  # Iterate through each "term" element
        ...
        id_map[node_name] = node_id  # Store ID for this term name
    for i in NAMESPACES_TO_TRACK:  # Iterate through each namespace to track
        if result[i]:  # Check if the namespace has any terms
            ...
            term_entries = [f"ID={id_map[k]}, GO{id_map[k][3:]}, Name={k}" for k in max_key]  # Format term IDs and names
            print(f"Max count for {i}: {max_count} for [{' | '.join(term_entries)}]")  # Print the maximum count and corresponding term entries
        else:
            print(f"Max count for {i}: 0 for []")  # Handle empty namespace
...
def parse_xml_sax(file):  # Parse the XML file using SAX parser and print the number of terms
    ...
    class CoHandler(xml.sax.ContentHandler):  # Define a custom SAX content handler
        def __init__(self):  # Initialize the handler
            ...
            self.current_id = ""  # Initialize the term ID
            self.is_a_count = 0  # Initialize the "is_a" count
            self.id_map = {}  # Map term names to IDs for output
        ...
        def startElement(self, name, attrs):  # Handle the start of an XML element
            ...
            if self.current_data == "term":  # If the element is a "term"
                ...
                self.current_id = ""  # Reset the term ID
        ...
        def characters(self, content):  # Handle the text content of an XML element
            ...
            elif self.current_data == "id":  # If the current element is "id"
                self.current_id += content  # Append the content to the term ID
        ...
        def endElement(self, name):  # Handle the end of an XML element
            if name == "name":  # If the element is "name"
            ...
            elif name == "id":  # If the element is "id"
                self.current_id = self.current_id.strip()  # Strip whitespace from the term ID
                ...
                if self.namespace in NAMESPACES_TO_TRACK and self.name:  # If the namespace is tracked and the name is valid
                    ...
                    self.id_map[self.name] = self.current_id  # Store ID for this term name
    ...
    for i in NAMESPACES_TO_TRACK:  # Iterate through each namespace to track
        ...
        if namespace_dict:  # Check if the namespace has any terms
            ...
            term_entries = [f"ID={handler.id_map[k]}, GO{handler.id_map[k][3:]}, Name={k}" for k in max_key]  # Format term IDs and names
            print(f"Max count for {i}: {max_count} for [{' | '.join(term_entries)}]")  # Print the maximum count and corresponding term entries
        else:
            print(f"Max count for {i}: 0 for []")  # Handle empty namespace
